...
clean_df_long <- combined_df_na %>%
  filter(!is.na(num_doc_degrees_2020_2021), !is.na(num_doc_degrees_2021_2022), !is.na(num_doc_degrees_2022_2023), !is.na(research_tier_2025)) %>%
  pivot_longer(cols = starts_with("num_doc_degrees"), 
  ...
  mutate(
    research_tier_2025 = recode(
      ...
      "Research 2: High Research Spending and Doctorate Production" = "R2",
      "Research Colleges and Universities" = "Research Colleges\nand Universities",
      academic_year = gsub("num_doc_degrees_", "", academic_year)
    )
  )

# Group by academic year and research tier to calculate the average number of doctoral degrees
line_plot_data <- clean_df_long %>%
  group_by(academic_year, research_tier_2025) %>%
  summarize(avg_doc_degrees = mean(num_doc_degrees, na.rm = TRUE))

# Line plot by academic year for each research tier
ggplot(line_plot_data, aes(x = academic_year, y = avg_doc_degrees, color = research_tier_2025, group = research_tier_2025)) +
  geom_line(size = 1.2) + 
  geom_point(size = 3) + 
  labs(
    title = "Trend of Average Doctoral Degrees by Research Tier (2020–2023)",
    x = "Academic Year",
    y = "Average Doctoral Degrees",
    color = "Research Tier",
    caption = "Note: 'Research Colleges and Universities' are research-focused but do not meet R1 or R2 criteria"
  ) +
  theme_minimal() +
