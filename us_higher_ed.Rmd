...
# INTERACTABLE: Raw Count of Institutions by College Type and Subcategory

```{r}
# Step 1: Prepare the data with counts
combined_df_na_cleaned_count <- combined_df_na_cleaned %>%
  group_by(college_type, subcategory) %>%
  summarise(count = n()) %>%
  ungroup()

# Step 2: Create the plot with counts for hover labels
count_collegetype_subcat <- ggplot(combined_df_na_cleaned_count, 
                                   aes(x = college_type, y = count, fill = subcategory, 
                                       text = paste("Category:", subcategory, "<br>Count:", count))) + 
  geom_bar(stat = "identity", position = "stack") +  # Use stat="identity" since we're passing counts directly
  scale_fill_manual(values = color_palette_shaded) +
  labs(
    title = "Distribution of U.S. Institutions by Carnegie Classification (2021)",
    subtitle = "Breakdown by college type, degree focus, and program size",
    x = "",
...
# Step 3: Convert to interactive plotly plot with hover text
interactive_plot <- ggplotly(count_collegetype_subcat, tooltip = "text") %>%
  layout(
    showlegend = FALSE,  # Removes legend in Plotly
    title = list(x = 0, xanchor = "left"),  # Left-align the title in Plotly
    annotations = list(
      x = 0, y = -0.25,  # Position the caption further down
      text = "College type determined by majority degrees conferred at the institution\nNote: 'Research Colleges and Universities' (RCU) are research-focused but do not meet R1 or R2 criteria for expenses and/or doctoral degrees conferred.\nData source: Carnegie Classification of Institutions of Higher Education (https://carnegieclassifications.acenet.edu)",
      showarrow = FALSE,
      xref = 'paper', yref = 'paper',
      xanchor = 'left', yanchor = 'top',
      font = list(size = 10)
    ),
    margin = list(b = 150)  # Increase bottom margin for more space
  )
...
# Bar graphs are stacked on top of eachother
```{r}
...
# Combine the two plots and add a single title, subtitle, and caption
...
# Emphasize trend and log scale
```{r}
# Reshape the data and apply a small constant to avoid log issues
long_combined_df_log <- long_combined_df %>%
  mutate(
    num_doc_degrees_log = ifelse(num_doc_degrees > 0, num_doc_degrees, NA) + 1  # Add 1 to avoid zero values, replace zero with NA
  ) %>%
  filter(!is.na(num_doc_degrees_log))  # Remove rows with NA after adding the constant

# Create a stacked bar graph with log-transformed y-axis
ggplot(long_combined_df_log, aes(x = academic_year, y = num_doc_degrees_log, fill = research_tier_2025)) +
  ...
  labs(
    ...
    subtitle = "Distribution of doctoral degrees awarded by R1, R2, and RCU institutions each year",
    x = "Academic year",
    y = "Log of total count (after adding 1)",
    fill = "",
    caption = "Note: 'Research Colleges and Universities' (RCU) are research-focused but do not meet R1 or R2 criteria for expenses and/or doctoral degrees conferred.\nData source: Carnegie Classification of Institutions of Higher Education (https://carnegieclassifications.acenet.edu)"
  ) +
  ...
  # Adjust the legend and color guide
  guides(
    fill = guide_legend(ncol = 1)  # Set the legend to 1 column
  ) +
  
  # Set the colors for the research tiers
  scale_fill_manual(
    values = c("Research 1" = "#4C79A1", "Research 2" = "#F1A340", "Research Colleges and Universities" = "#6D9D4B"),  # New colors
    labels = c("R1", "R2", "RCU")  # Set the legend labels
  ) +
  ...
  scale_y_continuous(
    trans = "log10",  # Log scale for the y-axis
    labels = scales::comma  # Adds commas to the y-axis labels for better readability
  )
...
ggplot(long_combined_df_log, aes(x = academic_year, y = num_doc_degrees_log, fill = research_tier_2025)) +
  ...
  labs(
    ...
    subtitle = "Distribution of doctoral degrees awarded by R1, R2, and RCU institutions each year",
    x = "Academic Year",
    y = "Log10 of Total Doctoral Degrees (count + 1)",
    fill = "",
    caption = "Note: 'Research Colleges and Universities' (RCU) are research-focused but do not meet R1 or R2 criteria for expenses and/or doctoral degrees conferred.\nData source: Carnegie Classification of Institutions of Higher Education (https://carnegieclassifications.acenet.edu)"
  ) +

  # Your custom plot theme
  my_plot_theme +
  theme(
    plot.title = element_text(hjust = 0),
  ...
  # Custom colors for each research tier
  scale_fill_manual(
    values = c(
      "Research 1" = "#4C79A1",
      "Research 2" = "#F1A340",
      "Research Colleges and Universities" = "#6D9D4B"
    ),
    labels = c("R1", "R2", "RCU")
  ) +

  # Log scale with readable axis labels
  scale_y_continuous(
    trans = "log10",
    labels = scales::comma
  )
...
# All the numbers
```{r}
# Recreate the aggregated data (if not already created)
aggregated_df <- long_combined_df %>%
  group_by(academic_year, research_tier_2025) %>%
  summarize(total_doc_degrees = sum(num_doc_degrees, na.rm = TRUE), .groups = "drop")

total_sums <- long_combined_df %>%
  group_by(academic_year) %>%
  summarize(total_sum = sum(num_doc_degrees, na.rm = TRUE))

# Plot with both sets of labels
ggplot(aggregated_df, aes(x = academic_year, y = total_doc_degrees, fill = research_tier_2025)) +
  geom_bar(stat = "identity", position = "stack", width = 0.8) +
  
  # Raw count labels inside each section (white)
  geom_text(aes(label = scales::comma(total_doc_degrees)),
            position = position_stack(vjust = 0.5),
            color = "white", size = 5, fontface = "bold") +
  
  # Total sum labels on top of each bar (black)
  geom_text(data = total_sums,
            aes(x = academic_year, y = total_sum, label = paste0("Total: ", scales::comma(total_sum))),
            inherit.aes = FALSE,
            vjust = .5,
            color = "black", size = 4.25, fontface = "bold",
            nudge_y = 7) +
  ...
  labs(
    ...
    subtitle = "Distribution of doctoral degrees awarded by R1, R2, and RCU institutions each year",
    x = "Academic Year",
    y = "Total Doctoral Degree Count (log scale on bar height; numbers are raw totals)",
    fill = "",
    caption = "Note: 'Research Colleges and Universities' (RCU) are research-focused but do not meet R1 or R2 criteria for expenses and/or doctoral degrees conferred.\nData source: Carnegie Classification of Institutions of Higher Education (https://carnegieclassifications.acenet.edu)"
  ) +
  ...
  scale_fill_manual(
    values = c("Research 1" = "#4C79A1", "Research 2" = "#F1A340", "Research Colleges and Universities" = "#6D9D4B"),
    labels = c("R1", "R2", "RCU")
  ) +
...
ggplot(plot_df_clean, aes(x = public_private_profit, y = n, fill = research_tier_2025)) +
  geom_bar(position = "dodge", stat = "identity", width = 0.6) +  # Adjust width for clarity
  geom_text(
    aes(label = n),            # Add labels with the count
    position = position_dodge(width = 0.6),   # Align text with bars
